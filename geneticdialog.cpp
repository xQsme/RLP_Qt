...
void GeneticDialog::clearLayout(){
    QLayoutItem *child;
...
void GeneticDialog::enableGraph()
{
    if(previous != 0)
    {
        clearLayout();
...
void GeneticDialog::enableThreads(){
    if(previous == 0)
    {
        done = 1;
    }
...
void GeneticDialog::on_pushButtonRead_clicked()
{
    if(ui->pushButtonRead->text() == "Solve")
    {
        ...
        if(fileName != "")
        {
            ...
            timer.start(1000);
            mainThread = new GeneticThread(fileName, ui->lineEditSeed->text().toInt(),
...
void GeneticDialog::on_pushButtonSolve_clicked()
{
    if(ui->pushButtonSolve->text() == "Batch Solve")
    {
        ...
        if(dir.dirName() != ".")
        {
            ...
            enableThreads();
            QFile info("../RLP_Qt/DataSets/" + dir.dirName() + "_genetic_algorithm_settings_" + ui->comboBoxSeeds->currentText() + ".txt");
...
void GeneticDialog::onDataChanged(QString stuff)
{
    QList<QString> moreStuff = stuff.split(" ");
...
void GeneticDialog::disableForm(int batch)
{
...
void GeneticDialog::newProblem(int thread, QString fileName, int percent)
{
...
void GeneticDialog::problemEnded(QString stuff, int ended)
{
    stream << stuff << endl;
    if(ended == 1)
    {
